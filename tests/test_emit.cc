...
std::shared_ptr<UnitNode> parse_unit(std::string path, std::string text,
  ...
  std::unique_ptr<RegistryParserActions> actions;
  if (registry) {
    actions = std::make_unique<RegistryParserActions>(*registry);
  }
  Parser p(lx, actions.get());
  return p.parse();
}
...
EmittedUnit compile_snippet_with_registry(
  ...
  if (!extra_units.empty()) {
    ...
    const std::filesystem::path dir =
        ...
        ("tp2cc-emit-" + std::to_string(++sequence));
    std::filesystem::remove_all(dir);
    std::filesystem::create_directories(dir);
    for (const auto& [path, unit_text] : extra_units) {
      const std::filesystem::path output = dir / path;
      std::filesystem::create_directories(output.parent_path());
      std::ofstream(output) << unit_text;
    }
    const std::filesystem::path entry = dir / "entry.pas";
    std::ofstream(entry) << text;

    UnitGraph graph;
    graph.add_search_root(dir);
    graph.discover_from_entry(entry);
    const UnitNode* root = nullptr;
    for (const auto& [_, parsed] : graph.units()) {
      if (parsed.path == entry && parsed.ast) {
        root = parsed.ast.get();
        break;
      }
    }
...
EmittedUnit compile_snippet_with_init_order(
  ...
  Lexer lx(std::move(sf));
  TypeRegistry reg;
  RegistryParserActions actions(reg);
  Parser p(lx, &actions);
  auto u = p.parse();
  if (!u) return {};
...
void test_program_runs_exitproc_before_returning_exitcode() {
  auto out = compile_snippet(
      "program demo;\n"
      "begin\n"
      "  ExitCode := 7;\n"
      "end.\n");
  const size_t body = out.impl.find("::rt::p_exitcode = 7;");
  const size_t exit_procs =
      out.impl.find("::rt::tp2cc_run_exit_procs();");
  const size_t result = out.impl.find("return ::rt::p_exitcode;");
  CHECK(body != std::string::npos);
  CHECK(exit_procs != std::string::npos);
  CHECK(result != std::string::npos);
  CHECK(body < exit_procs);
  CHECK(exit_procs < result);
  CHECK(!contains(out.impl, "return 0;"));
}
...
void test_enum_explicit_values_use_declaring_unit_scope() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "uses dep;\n"
      "type\n"
      "  tflag = (first := start, second);\n"
      "const\n"
      "  start = 99;\n"
      "implementation\n"
      "end.\n",
...
void test_unimported_qualified_enum_member_is_not_const_folded() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "const\n"
      "  folded = ord(other.red);\n"
      "implementation\n"
      "end.\n",
      {{"other.pas",
        "unit other;\n"
        ...
        "end.\n"}});
  CHECK(error_count() > before);
  CHECK(!contains(out.header, "p_folded = 0"));
}
...
void test_low_high_on_record_field_type_uses_declared_field_type() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "  end;\n"
      "const\n"
      "  invalid = low(tpos.val);\n"
      "implementation\n"
      "end.\n");
  CHECK(contains(out.header,
                 "p_invalid = ::std::numeric_limits<int32_t>::min();"));
  CHECK(!contains(out.header, "::rt::p_low("));
}
...
void test_low_high_subrange_bounds_use_declaring_unit_scope() {
  auto out = compile_snippet_with_registry(
      ...
      "var i : longint;\n"
      "begin\n"
      "  for i := low(dep.tsmall) to high(dep.tsmall) do begin end;\n"
      "end;\n"
      "end.\n",
      {{"dep.pas",
        "unit dep;\n"
        ...
        "implementation\n"
        "end.\n"}});
  CHECK(contains(out.impl, "auto tp2cc_to = (::p_dep::p_limit);"));
  CHECK(!contains(out.impl, "auto tp2cc_to = (p_limit);"));
  CHECK(!contains(out.impl, "::rt::p_high("));
}
...
void test_non_system_qualified_low_call_stays_unit_call() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      {{"helper.pas",
        ...
        "end.\n"}});
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "::p_helper::p_low("));
  CHECK(!contains(out.impl, "::rt::p_low("));
...
void test_type_alias_does_not_emit_backend_carrier() {
  auto out = compile_snippet(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "implementation\n"
      "end.\n");
  CHECK(!contains(out.header, "using t_myint"));
...
void test_build_resolves_alias_descriptor_identity() {
  ...
  const TypeSymbol* x = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                     pascal_key("x"));
  const TypeSymbol* y = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                     pascal_key("y"));
  CHECK(x && y);
  ...
  CHECK_EQ(y_decl->type->descriptor, x->descriptor);
...
void test_build_resolves_metaclass_alias_descriptor_identity() {
  ...
  const TypeSymbol* foo = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                       pascal_key("tfoo"));
  const TypeSymbol* ta = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                      pascal_key("ta"));
  const TypeSymbol* tb = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                      pascal_key("tb"));
  CHECK(foo && ta && tb);
...
void test_runtime_tclass_is_rt_metaclass_alias_not_builtin_atom() {
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
      "unit u;\n"
  ...
  const TypeSymbol* tclass = reg.lookup_type_symbol_exact(pascal_key("__rt__"),
                                                          pascal_key("tclass"));
  const TypeSymbol* tobject = reg.lookup_type_symbol_exact(pascal_key("__rt__"),
                                                           pascal_key("tobject"));
  CHECK(tclass && tobject);
  CHECK_EQ(reg.builtin_literal("tclass"), nullptr);
  CHECK_EQ(tclass->defining_unit, "__rt__");
  CHECK_EQ(tclass->descriptor->metaclass_target, tobject);
}

void test_runtime_alias_types_come_from_registry_tree() {
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
      "unit u;\n"
  ...
  const TypeExpr* runtime_type =
      reg.lookup_type_symbol_exact(pascal_key("__rt__"),
                                   pascal_key("tmethod"))->type;
  const TypeSymbol* symbol = reg.resolved_symbol_for_type(runtime_type);
  CHECK(symbol != nullptr);
  CHECK_EQ(symbol->defining_unit, "__rt__");
  CHECK_EQ(symbol->name, "tmethod");

  TypeRegistry second_registry;
  auto second_unit = parse_unit(
      "<mem>",
      "unit second;\n"
      "interface\n"
      "implementation\n"
      "end.\n",
      &second_registry);
  const TypeExpr* second_type =
      second_registry.lookup_type_symbol_exact(
          pascal_key("__rt__"), pascal_key("tmethod"))->type;
  CHECK(second_type != runtime_type);
  CHECK_EQ(reg.resolved_symbol_for_type(runtime_type), symbol);
  CHECK_EQ(second_registry.resolved_symbol_for_type(second_type)->name,
           "tmethod");

  TyName raw(Location{}, "tmethod");
  CHECK_EQ(reg.resolved_symbol_for_type(&raw), nullptr);

  const TypeSymbol* heap_error = reg.lookup_type_symbol_exact(
      pascal_key("__rt__"), pascal_key("eheapmemoryerror"));
  const TypeSymbol* heap_exception = reg.lookup_type_symbol_exact(
      pascal_key("__rt__"), pascal_key("eheapexception"));
  CHECK(heap_error && heap_exception);
  CHECK_EQ(heap_exception ? heap_exception->descriptor : nullptr,
           heap_error ? heap_error->descriptor : nullptr);
}

void test_inferred_types_are_registry_owned() {
  TypeRegistry first;
  auto first_unit = parse_unit(
      "<mem>",
      "unit first;\n"
      "interface\n"
      "implementation\n"
      "end.\n",
      &first);
  const TypeExpr* longint_type = first.builtin_literal("longint")->type;
  const TySet* first_set =
      first.inferred_set_type(longint_type, std::make_pair(0, 31));
  const TyPointer* first_pointer =
      first.inferred_pointer_type(longint_type);
  CHECK(first_set && first_set->descriptor);
  CHECK(first_pointer && first_pointer->descriptor);
  CHECK_EQ(first_pointer->descriptor->pointer_difference_result,
           first.builtin_literal("ptrint")->descriptor);
  CHECK_EQ(first.inferred_set_type(longint_type, std::make_pair(0, 31)),
           first_set);
  CHECK_EQ(first.inferred_pointer_type(longint_type), first_pointer);
  ...
  auto second_unit = parse_unit(
      ...
      "end.\n",
      &second);
  const TypeExpr* second_longint =
      second.builtin_literal("longint")->type;
  CHECK(second_longint != longint_type);
  CHECK(second.inferred_set_type(second_longint, std::make_pair(0, 31)) !=
        first_set);
}

void test_descriptor_owned_semantic_indexes() {
  TypeRegistry reg;
  auto unit = parse_unit(
      ...
      "var value : record field : longint; end;\n"
      "const flag = true;\n"
      "      letter = 'x';\n"
      "      compared = 1 < 2;\n"
      "      converted = longint(3);\n"
  ...
  const TypeSymbol* color = reg.lookup_type_symbol_exact(
      pascal_key("u"), pascal_key("tcolor"));
  const EnumInfoReg* color_info = color ? color->enum_info() : nullptr;
  auto unit_info = reg.units.find("u");
  CHECK(color_info);
  CHECK(unit_info != reg.units.end());
  if (unit_info != reg.units.end()) {
    auto red = unit_info->second.iface_enum_members.find("red");
  ...
  const EnumMemberInfo* fast_red =
      reg.lookup_enum_member_in_unit("u", "red");
  CHECK(fast_red);
  CHECK_EQ(fast_red ? fast_red->owner : nullptr, color_info);

  auto* variable =
      unit && unit->interface_decls.size() > 1
          ? dynamic_cast<VarDecl*>(unit->interface_decls[1].get())
          : nullptr;
  const TypeDescriptor* anonymous_record =
      reg.descriptor_for_type(variable ? variable->type.get() : nullptr);
  CHECK(anonymous_record);
  CHECK(anonymous_record && anonymous_record->record_info());
  CHECK(anonymous_record && anonymous_record->record_info() &&
        anonymous_record->record_info()->fields.count("field") == 1);

  const TypeDescriptor* boolean =
  ...
  CHECK_EQ(reg.expression_result_descriptor(
           ...
           boolean);
  CHECK_EQ(reg.expression_result_descriptor(
               static_cast<ConstDecl&>(*unit->interface_decls[3]).value.get()),
           character);
  CHECK_EQ(reg.expression_result_descriptor(
  ...
  CHECK(integer_alias && longint_symbol);
  CHECK_EQ(reg.inferred_pointer_type(integer_alias->type),
           reg.inferred_pointer_type(longint_symbol->type));
  CHECK_EQ(reg.inferred_set_type(integer_alias->type, std::make_pair(0, 7)),
           reg.inferred_set_type(longint_symbol->type, std::make_pair(0, 7)));
}
...
void test_build_resolves_pointer_forward_descriptor_identity() {
  ...
  const TypeSymbol* tfoo = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                        pascal_key("tfoo"));
  ...
  CHECK_EQ(reg.descriptor_for_type(pointer ? pointer->target.get() : nullptr),
           tfoo->descriptor);
  CHECK_EQ(pointer && pointer->target ? pointer->target->descriptor : nullptr,
           tfoo->descriptor);
}
...
void test_build_class_forward_completion_can_cross_type_sections() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_class_forward_completion_must_be_class_body() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_class_field_uses_prior_forward_class_decl() {
  ...
  const TypeSymbol* section = reg.lookup_type_symbol_exact(
      pascal_key("u"), pascal_key("tsection"));
  const TypeSymbol* symbol = reg.lookup_type_symbol_exact(
      pascal_key("u"), pascal_key("tsymbol"));
...
void test_build_rejects_implementation_type_redeclaring_interface_type() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
      ...
      "type\n"
      "  TName = record\n"
      "    FromInterface : LongInt;\n"
      "  end;\n"
      "implementation\n"
      "type\n"
      "  TName = record\n"
      "    FromImplementation : LongInt;\n"
      "  end;\n"
      "end.\n",
      &reg);
  CHECK(error_count() > before);

  const TypeSymbol* name = reg.lookup_type_symbol_exact(
      pascal_key("u"), pascal_key("tname"));
  const RecordInfo* record = name ? name->record_info() : nullptr;
  CHECK(record);
  CHECK(record && record->fields.count("frominterface") == 1);
  CHECK(record && record->fields.count("fromimplementation") == 0);
}
...
void test_build_var_type_does_not_see_later_type_section() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_proc_local_var_type_does_not_see_later_type_section() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_rejects_proc_local_duplicate_type() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_rejects_proc_local_duplicate_enum_label() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_rejects_proc_local_enum_label_redeclared_by_later_type() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_rejects_proc_local_enum_label_conflicting_with_value_decl() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_rejects_unit_enum_label_conflicting_with_value_decl() {
  int before = error_count();
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
...
void test_build_resolves_set_element_descriptor_identity() {
  ...
  const TypeSymbol* tenum = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                         pascal_key("tenum"));
  ...
  CHECK_EQ(set_type && set_type->element
               ? set_type->element->descriptor
               : nullptr,
           tenum->descriptor);
}
...
void test_build_resolves_builtin_descriptor_identity() {
  ...
  CHECK(atom && atom->descriptor && atom->descriptor->primitive);
  CHECK(atom && atom->descriptor && atom->descriptor->primitive &&
        atom->descriptor->primitive->kind == PrimitiveKind::LongInt);
...
void test_builtin_pascal_aliases_share_declared_target_identity() {
  TypeRegistry reg;
  (void)parse_unit(
      "<mem>",
      "unit u;\n"
  ...
  const auto descriptor = [&](std::string_view name) {
    const TypeSymbol* symbol = reg.builtin_literal(name);
    CHECK(symbol && symbol->descriptor);
    return symbol ? symbol->descriptor : nullptr;
  };

  CHECK_EQ(descriptor("integer"), descriptor("longint"));
  CHECK_EQ(descriptor("cardinal"), descriptor("longword"));
  CHECK_EQ(descriptor("dword"), descriptor("longword"));
  CHECK_EQ(descriptor("ansichar"), descriptor("char"));
  CHECK_EQ(descriptor("pansichar"), descriptor("pchar"));
}
...
void test_ansistring_builtin_maps_to_runtime_type() {
  auto out = compile_snippet(
      ...
      "implementation\n"
      "end.\n");
  CHECK(!contains(out.header, "using t_tname"));
...
void test_widechar_builtin_maps_to_16bit_ordinal() {
  auto out = compile_snippet(
      ...
      "implementation\n"
      "end.\n");
  CHECK(!contains(out.header, "using t_twide"));
...
void test_ansichar_and_pansichar_builtin_maps_to_char_carriers() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  CHECK(!contains(out.header, "using t_ta"));
  CHECK(!contains(out.header, "using t_tpa"));
...
void test_ord_storage_view_for_char_assignment_inc_and_address() {
  ...
  CHECK(error_count() > before);
  CHECK(contains(out.impl, "static_cast<uint8_t*>(nullptr)"));
...
void test_chr_storage_view_for_byte_assignment_and_inc() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "procedure run;\n"
...
void test_imported_default_argument_does_not_see_caller_result() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      {{"api.pas",
        ...
        "end.\n"}});
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "::p_api::p_take(::p_api::p_marker);"));
  CHECK(!contains(out.impl, "::p_api::p_take(p_result);"));
}
...
void test_unit_qualified_value_type_guides_overload_selection() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      {{"dep.pas",
        ...
        "end.\n"}});
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "::p_dep::p_b"));
  CHECK(contains(out.impl, "::p_dep::p_c"));
  CHECK(!contains(out.impl, "static_cast<int32_t>(::p_dep::p_b"));
  CHECK(!contains(out.impl, "static_cast<int32_t>(::p_dep::p_c"));
}
...
void test_build_binds_empty_exception_handler_to_runtime_exception() {
  TypeRegistry reg;
  auto u = parse_unit(
      "<mem>",
      "unit u;\n"
  ...
  ProcDecl* demo = nullptr;
  for (const DeclPtr& decl : u->impl_decls) {
    auto* proc = dynamic_cast<ProcDecl*>(decl.get());
    if (proc && proc->name == "demo" && proc->body) {
      demo = proc;
      break;
    }
  }
  ...
  auto* body = dynamic_cast<Compound*>(demo->body.get());
  CHECK(body && !body->body.empty());
  if (!body || body->body.empty()) return;
  auto* try_stmt = dynamic_cast<Try*>(body->body.front().get());
  CHECK(try_stmt && try_stmt->handlers.size() == 1);
  if (!try_stmt || try_stmt->handlers.size() != 1) return;

  ExceptHandler& handler = try_stmt->handlers.front();
  handler.class_name.clear();

  std::optional<const TypeSymbol*> result =
      reg.exception_handler_type_result(&handler);
  CHECK(result && *result);
  const TypeSymbol* runtime_exception =
      reg.lookup_type_symbol_exact(pascal_key("__rt__"),
                                   pascal_key("exception"));
  CHECK(runtime_exception);
  CHECK_EQ(result && *result ? *result : nullptr, runtime_exception);
}
...
void test_sysutils_exception_handlers_are_qualified_and_pointer_bound() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  CHECK(contains(out.impl, "dynamic_cast<::rt::t_eoutofmemory*>("));
  CHECK(contains(out.impl, "dynamic_cast<::rt::t_einouterror*>("));
...
void test_shortstring_assignment_uses_pascal_string_helper() {
  ...
  CHECK(contains(
      out.impl,
      "::rt::tp2cc_shortstring_assign(::rt::tp2cc_ShortStringPtrRef<255>{reinterpret_cast<uint8_t*>(p_p)}, p_s);"));
  CHECK(!contains(out.impl, "::rt::tp2cc_deref(p_p)"));
}

void test_shortstring_pointer_cast_reads_only_length_prefixed_storage() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
  ...
  CHECK(contains(
      out.impl,
      "::rt::tp2cc_ShortStringPtrRef<255>{reinterpret_cast<uint8_t*>("));
  CHECK(contains(out.impl, "p_bytes[p_i]"));
  CHECK(!contains(out.impl,
                  "tp2cc_reinterpret_load<::rt::tp2cc_ShortString"));
}
...
void test_var_shortstring_capacity_mismatch_uses_storage_ref() {
  ...
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK_EQ(error_count(), before);
  CHECK(!contains(out.impl, "no matching call to 'replace'"));
  CHECK(contains(out.header,
...
void test_implementation_operator_sees_interface_operator_overload() {
  auto out = compile_snippet_with_registry(
      "unit ops;\n"
      "interface\n"
      "type\n"
  ...
  CHECK(!contains(out.impl, "no matching call to 'operator +'"));
  CHECK(contains(out.impl, "p_b = (p_a + p_a);"));
  CHECK(contains(out.impl, "p_b = (p_a + 1);"));
}
...
void test_untyped_file_var_method_accepts_file_actual() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK_EQ(error_count(), before);
  CHECK(!contains(out.impl, "no matching call to 'blockwrite'"));
}

void test_file_kinds_use_distinct_runtime_carriers() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "var\n"
      ...
      "implementation\n"
      "end.\n");
  CHECK(contains(out.header, "::rt::tp2cc_TextFile p_textfile"));
  CHECK(contains(out.header, "::rt::tp2cc_UntypedFile p_untypedfile"));
  CHECK(contains(out.header, "::rt::tp2cc_TypedFile<uint8_t> p_typedfile"));
}
...
void test_byte_array_typecast_index_read_builds_value() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  // `t80bitarray(e)[i]` reads one byte of `e`'s representation. Copying
  // the whole 10-byte array first would be wasteful and would still leave
  // the aliasing question open; a single-byte memcpy-load at the indexed
  // offset is both defined and minimal.
  CHECK(contains(out.impl, "::rt::tp2cc_reinterpret_load<uint8_t>("));
  CHECK(contains(out.impl, "::rt::tp2cc_byte_offset((&p_e)"));
...
void test_local_byte_array_typecast_index_read_builds_value() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  // Same reasoning as `test_byte_array_typecast_index_read_builds_value`.
  CHECK(contains(out.impl, "::rt::tp2cc_reinterpret_load<uint8_t>("));
  CHECK(contains(out.impl, "::rt::tp2cc_byte_offset((&p_e)"));
  CHECK(!contains(out.impl, "t_t80bitarray(p_e)[p_i]"));
}
...
void test_text_typecast_over_pointer_deref_keeps_file_lvalue() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  // `text(arg^)` names TextFile bytes at `arg`'s target. `write`/`writeln`
  // need a real `TextFile&` at that address so Pascal identity is preserved.
  CHECK(contains(out.impl,
                 "::rt::p_write(::rt::tp2cc_storage_ref<::rt::tp2cc_TextFile>("));
  CHECK(contains(out.impl,
                 "::rt::p_writeln(::rt::tp2cc_storage_ref<::rt::tp2cc_TextFile>("));
...
void test_local_pointer_alias_cast_uses_local_type_name() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  // `psetbytes(raw)^[0]` deref-and-index through a typed pointer cast on
  // untyped storage. Reading `*(psetbytes)raw` as a typed C++ lvalue would
  // be strict-aliasing UB; route the byte at the requested index through
  // memcpy instead.
  CHECK(contains(out.impl, "::rt::tp2cc_reinterpret_load<uint8_t>("));
  CHECK(contains(out.impl, "::rt::tp2cc_byte_offset("));
  CHECK(!contains(out.impl,
...
void test_runtime_alias_type_names_are_explicitly_qualified() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  CHECK(contains(out.header, "extern ::rt::tp2cc_ShortString<> p_d;"));
...
void test_tdatetime_and_runtime_date_time_lower_through_rt() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  CHECK(contains(out.impl, "double p_dt{};"));
...
void test_runtime_aliases_cover_currency_systemtime_and_pansistring() {
  ...
  CHECK(contains(out.impl, "int32_t p_hr{};"));
...
void test_system_pointer_aliases_are_implicit_source_types() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK(error_count() == before);
  CHECK(contains(out.impl, "::rt::t_pshortstring p_ps{};"));
  CHECK(contains(out.impl, "::rt::t_ppointer p_pp{};"));
  CHECK(contains(out.impl, "::rt::t_pdword p_pd{};"));
}

void test_system_pointer_aliases_bind_in_interface_declarations() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK(error_count() == before);
  CHECK(!contains(out.header, "unresolved type `pshortstring`"));
  CHECK(!contains(out.header, "unresolved type `ppointer`"));
  CHECK(contains(out.header, "::rt::t_pshortstring p_name;"));
  CHECK(contains(out.header, "::rt::t_ppointer p_data;"));
  CHECK(contains(out.header, "::rt::t_pshortstring p_alt;"));
  CHECK(contains(out.header,
                 "void p_take(::rt::t_pshortstring &p_p, "
                 "::rt::t_ppointer p_q);"));
}
...
void test_string_comparison_uses_runtime_operator_resolution() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "::rt::tp2cc_string_compare(p_s, ::rt::tp2cc_ShortStringPtrRef<255>{reinterpret_cast<uint8_t*>(p_ps)}) != 0"));
  CHECK(contains(out.impl,
                 "::rt::tp2cc_string_compare(::rt::tp2cc_ShortStringPtrRef<255>{reinterpret_cast<uint8_t*>(p_ps)}, p_s) == 0"));
  CHECK(!contains(out.impl, "::rt::tp2cc_deref(p_ps)"));
}
...
void test_sizeof_visible_type_uses_type_name_not_identifier_lookup() {
  ...
  CHECK(!contains(out.header, "using t_aint"));
...
void test_unit_storage_duplicates_report_error() {
  int before = error_count();
  (void)compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_duplicate_type_members_report_error() {
  int before = error_count();
  (void)compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_unit_qualified_type_does_not_see_local_type_scope() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_absolute_pointer_target_reinterprets_pointer_slot_storage() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "tp2cc_ScopedStorageView<t_titem, void*>((&(p_raw)), (&(p_raw)))"));
  CHECK(!contains(out.impl, "t_titem &p_item"));
}
...
void test_absolute_pointer_alias_reinterprets_pointer_storage() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "tp2cc_reinterpret_load<t_pint>((&(p_raw)))"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_load<t_pint>(p_raw)"));
  CHECK(!contains(out.impl, "t_pint &p_value"));
}

void test_absolute_class_alias_over_pointer_parameter_reads_pointer_slot() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
  ...
  CHECK(contains(
      out.impl,
      "tp2cc_ScopedStorageView<t_titem*, void*>((&(p_arg)), (&(p_arg)))"));
  CHECK(!contains(out.impl, "tp2cc_storage_ref<t_titem*>(p_arg)"));
}
...
void test_absolute_typed_const_alias_reinterprets_same_storage() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "::rt::tp2cc_reinterpret_load<t_pint>(::rt::tp2cc_byte_offset((&(p_raw)), ((0) - (0)) * sizeof(t_pint)))"));
  CHECK(!contains(out.impl, "t_parr &p_view"));
}
...
void test_absolute_const_param_alias_stays_const_reference() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "::rt::tp2cc_reinterpret_load<::rt::tp2cc_ShortString<>>((&(p_s)))"));
  CHECK(!contains(out.impl, "p_view"));
}
...
void test_tobject_runtime_helpers_lower_in_method_body() {
  ...
  CHECK(contains(out.impl, "p_result = static_cast<void*>(p_classtype());"));
...
void test_method_pointer_type_and_bound_assignment_emit() {
  ...
  CHECK(contains(out.header,
                 "static void tp2cc_methodptr_fire_value_name_longint_ret_void"));
  CHECK(contains(out.impl,
                 "::rt::tp2cc_method_code<&t_tobj::tp2cc_methodptr_fire_value_name_longint_ret_void"));
...
void test_method_pointer_record_cast_reinterprets_same_storage() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "::rt::tp2cc_reinterpret_store<void*>(::rt::tp2cc_byte_offset((&p_p), offsetof(t_trec, p_procpointer)), p_addr);"));
  CHECK(contains(out.impl,
                 "::rt::tp2cc_reinterpret_store<void*>(::rt::tp2cc_byte_offset((&p_p), offsetof(t_trec, p_s)), p_selfp);"));
}
...
void test_unbound_method_address_uses_thunk_code() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.header,
                 "p_addr = ::rt::tp2cc_method_code<&t_tobj::tp2cc_methodptr_fire_value_name_longint_ret_void"));
}
...
void test_explicit_plain_proc_callback_argument_uses_pointer_carrier_adapter() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "procedure run(list:tlist);\n"
      "begin\n"
      "  list.foreachcall(tlistcallback(@visit), nil);\n"
      "end;\n"
      "end.\n");
  CHECK(contains(out.header, "using t_tlistcallback = void (*)(void*, void*);"));
  CHECK(contains(out.impl,
                 "p_list->p_foreachcall(+[](void* p_p, void* p_arg) -> void { "
                 "p_visit(static_cast<t_titem*>(p_p), p_arg); }, nullptr);"));
  CHECK(!contains(out.impl, "p_foreachcall((&p_visit)"));
}

void test_plain_proc_overload_prefers_exact_callback_signature() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end;\n"
      "end.\n");
  CHECK(contains(out.impl, "p_take(+[](void* p_p, void* p_arg) -> void { "
                           "p_visit(static_cast<t_titem*>(p_p), p_arg); })"));
}

void test_plain_proc_mismatched_callback_argument_needs_explicit_cast() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK(error_count() > before);
  CHECK(contains(out.impl, "no matching call to 'foreachcall'"));
...
void test_var_param_accepts_same_pointee_pointer_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_param_accepts_nested_same_pointee_pointer_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_param_accepts_imported_same_pointee_pointer_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "uses second;\n"
      "procedure take(var p : Q);\n"
      "var\n"
      "  x : P;\n"
      "implementation\n"
      "procedure take(var p : Q);\n"
...
void test_var_param_accepts_same_domain_subrange_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_param_accepts_same_shape_fixed_array_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_param_accepts_same_shape_shortstring_array_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_param_rejects_different_shortstring_array_capacity() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_param_accepts_same_shape_set_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_object_pointer_formal_uses_declaring_unit_context() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK(!contains(out.impl, "no matching call to 'take'"));
  CHECK(contains(out.impl,
                 "::p_dep::p_take(static_cast<::p_dep::t_tbase*>(p_p));"));
}

void test_var_param_rejects_pointer_alias_to_distinct_record() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_single_candidate_call_accepts_derived_class_argument() {
  ...
  CHECK_EQ(error_count(), before);
  CHECK(!contains(out.impl, "no matching call to 'take'"));
  CHECK(contains(out.impl, "p_take(static_cast<t_tbase*>(p_c));"));
}
...
void test_single_candidate_call_accepts_constructor_result_as_tobject() {
  ...
  CHECK(!contains(out.impl, "no matching call to 'take'"));
  CHECK(contains(out.impl,
                 "p_take(static_cast<::rt::t_tobject*>(([&]{ auto "
                 "tp2cc_ptr = new t_tnode{};"));
...
void test_single_candidate_call_accepts_enum_member_for_subrange() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK_EQ(error_count() - before, 0);
  CHECK(!contains(out.impl, "no matching call to 'location_reset'"));
}
...
void test_out_typed_pointer_accepts_pointer_storage_slot() {
  ...
  CHECK(contains(
      out.impl,
      "tp2cc_ScopedStorageView<t_pwide, void*>((&p_value.p_valueptr), "
      "(&p_value.p_valueptr))"));
}
...
void test_record_cast_to_method_pointer_reports_error() {
  ...
  CHECK(error_count() > before);
  CHECK(!contains(out.impl, "::rt::tp2cc_reinterpret_copy<"));
  CHECK(!contains(out.impl, "p_result = p_r();"));
}
...
void test_open_array_method_signature_keeps_wrapper_type() {
  ...
  CHECK(contains(out.header,
                 "static ::rt::tp2cc_ShortString<> tp2cc_methodptr_get_value_name_longint_const_openarr_name_shortstring_ret_name_shortstring(void* tp2cc_self, int32_t p_nr, ::rt::tp2cc_OpenArray<::rt::tp2cc_ShortString<>> p_args)"));
}
...
void test_memory_helpers_reinterpret_typecast_pointer_slots() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "::rt::p_getmem((*::rt::tp2cc_ScopedStorageView<t_pdata, void*>((&p_raw), (&p_raw))), 4);"));
  CHECK(contains(out.impl,
...
void test_imported_enum_member_lowers_with_defining_unit_prefix() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "uses colors;\n"
      ...
      {{"colors.pas",
        ...
        "implementation\n"
        "end.\n"}});
  CHECK(contains(out.impl, "p_c = ::p_colors::p_red;"));
  CHECK(contains(out.impl, "(p_c == ::p_colors::p_blue)"));
  CHECK(!contains(out.impl, "p_c = p_red;"));
  CHECK(!contains(out.impl, "(p_c == p_blue)"));
}
...
void test_compatible_set_actual_stays_viable_in_overload_resolution() {
  ...
  CHECK_EQ(error_count() - before, 0);
  CHECK(contains(out.impl,
                 "p_u::p_take(::rt::tp2cc_set_cast<t_tbytes>(p_s))"));
}
...
void test_shadowed_prefetch_statement_calls_user_proc() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK(error_count() == before);
  CHECK(contains(out.impl, "p_prefetch(p_x);"));
}
...
void test_real_division_uses_registered_real_result_type() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK(contains(
      out.impl,
      "::rt::tp2cc_real_to_int_trunc<::rt::t_currency>"
      "((static_cast<double>(p_c) / 10000))"));
  CHECK(contains(out.impl, "p_c = (p_a / p_b);"));
  CHECK(!contains(out.impl, "static_cast<double>(p_a)"));
  CHECK(!contains(out.impl, "(p_c / 10000)"));
}

void test_real_division_preserves_floating_operand_result_type() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK(!contains(out.impl, "static_cast<double>((p_a / p_b))"));
}
...
void test_untyped_const_pointer_cast_from_integer_passes_pointed_address() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "procedure sink(const b; len : longint);\n"
...
void test_untyped_const_pointer_cast_deref_from_integer_passes_pointee_storage() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "procedure sink(const b; len : longint);\n"
...
void test_untyped_const_pointer_self_deref_passes_receiver_storage() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "p_move(((const void*)(this)), ((void*)(p_p)), 0);"));
  CHECK(!contains(out.impl, "(&this)"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_load<void*>"));
}

void test_pointer_cast_deref_from_integer_loads_pointee_storage() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type plongint = ^longint;\n"
...
void test_forward_pointer_to_set_emits_set_carrier_first() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "implementation\n"
      "end.\n");
  const size_t set_decl = out.header.find("using t_tconstset = ");
  const size_t pointer_decl = out.header.find("using t_pconstset = ");
  CHECK(set_decl != std::string::npos);
  CHECK(pointer_decl != std::string::npos);
  CHECK(set_decl < pointer_decl);
}
...
void test_tclass_alias_lowers_through_rt() {
  ...
  CHECK(contains(
      out.header,
      "bool p_sameclass(::rt::tp2cc_metaclass_t_tobject* p_c);"));
}
...
void test_interface_parameterless_method_result_autocalls_as_argument() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "p_decorator->p_lineprefix()"));
  CHECK(!contains(out.impl, "no matching call to 'writeansistring'"));
}
...
void test_class_actual_rejects_same_leaf_nested_interface_formal() {
  int before = error_count();
  (void)compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_record_field_does_not_see_later_nested_type() {
  int before = error_count();
  (void)compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_class_member_does_not_see_later_nested_type() {
  int before = error_count();
  (void)compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_metaclass_alias_and_concrete_class_value_lowering() {
  auto out = compile_snippet_with_registry(
      ...
      "    inst := cls.create(2);\n"
      "end.\n");
  CHECK(!contains(out.header, "using t_tbaseclass"));
  CHECK(contains(out.header,
...
void test_class_value_rejects_unrelated_class_of_target() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK(error_count() > before);
  CHECK(contains(out.impl, "p_takebase(tp2cc_metaclass_value_t_tchild());"));
  CHECK(contains(out.impl, "no matching call to 'takebase'"));
  CHECK(!contains(out.impl,
                  "p_takebase(tp2cc_metaclass_value_t_tunrelated());"));
  CHECK(!contains(out.impl, "no matching call to 'takeany'"));
  CHECK(contains(out.impl, "p_takeany(tp2cc_metaclass_value_t_tunrelated());"));
}
...
void test_metaclass_cast_keeps_concrete_descriptor() {
  ...
  CHECK(contains(out.impl, "p_childcls = "));
  CHECK(contains(out.impl,
                 "<tp2cc_metaclass_t_tchild*>(p_basecls);"));
  CHECK(!contains(out.impl,
                  "<tp2cc_metaclass_t_tbase*>(p_basecls);"));
}
...
void test_metaclass_aliases_to_same_target_share_type_identity() {
  ...
  CHECK(!contains(out.impl, "no matching call to 'take'"));
  CHECK(!contains(out.header, "using t_ta"));
  CHECK(!contains(out.header, "using t_tb"));
  CHECK(contains(out.header, "tp2cc_metaclass_t_tfoo*"));
}
...
void test_include_mutates_variant_payload_set_storage() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "p_include(::rt::tp2cc_storage_ref<"));
  CHECK(!contains(out.impl, "p_include(::rt::tp2cc_reinterpret_load"));
}

void test_include_mutates_typed_pointer_cast_field_set_storage() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl,
                 "p_include(::rt::tp2cc_deref(static_cast<t_ptaiprop>("));
  CHECK(contains(out.impl, ".p_usedregs"));
  CHECK(!contains(out.impl, "p_include(::rt::tp2cc_storage_ref<t_tregset>"));
  CHECK(!contains(out.impl, "p_include(::rt::tp2cc_reinterpret_load<t_tregset>"));
}
...
void test_variant_record_pointer_payload_passes_slot_to_allocation_builtins() {
  ...
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl,
                 "::rt::p_new(::rt::tp2cc_storage_ref<t_pint>(::rt::tp2cc_byte_offset((&p_view), offsetof(t_tview, p_item))));"));
  CHECK(contains(out.impl,
                 "::rt::p_getmem(::rt::tp2cc_storage_ref<::rt::p_char*>(::rt::tp2cc_byte_offset((&p_view), offsetof(t_tview, p_text))), p_size);"));
...
void test_shadowed_getmem_does_not_use_runtime_slot_helper() {
  ...
  CHECK(contains(out.impl,
                 "p_getmem(::rt::tp2cc_storage_ref<::rt::p_char*>(::rt::tp2cc_byte_offset((&p_view), offsetof(t_tview, p_text))), p_size);"));
}
...
void test_variant_record_pointer_payload_typecast_keeps_slot_storage() {
  ...
  CHECK(contains(out.impl,
                 "p_touch(::rt::tp2cc_storage_ref<t_pwide>(::rt::tp2cc_byte_offset((&p_value), offsetof(t_tconstvalue, p_valueptr))));"));
  CHECK(!contains(out.impl,
...
void test_record_typecast_field_read_uses_storage_view() {
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "::rt::tp2cc_reinterpret_load<uint32_t>(::rt::tp2cc_byte_offset((&p_x), offsetof(t_twoword, p_hi)))"));
  ...
  auto out = compile_snippet_with_registry(
      ...
      {{"first.pas",
       ...
       {"second.pas",
        ...
        "end;\n"
        "end.\n"}});
  CHECK(contains(out.impl, "::p_first::p_pick("));
...
void test_class_overload_continuation_matches_fpc_any_procdef_rule() {
  ...
  const TypeSymbol* child = reg.lookup_type_symbol_exact(pascal_key("u"),
                                                         pascal_key("tchild"));
  const auto* methods = child && child->class_info()
                            ? reg.lookup_class_methods(*child->class_info(),
                                                       "pick")
                            : nullptr;
  CHECK(methods);
...
void test_bare_proc_type_does_not_see_hidden_imported_function() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK(error_count() > before);
  CHECK(contains(out.impl, "no matching call to 'take'"));
  CHECK(!contains(out.impl, "p_other::p_hidden"));
}
...
void test_with_field_type_shadows_current_class_for_overload() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "p_take(static_cast<uint8_t>(tp2cc_with_0->p_value));"));
  CHECK(!contains(out.impl, "p_take(static_cast<int32_t>"));
}
...
void test_inline_anon_enum_var_name_is_not_type_name() {
  int before = error_count();
  (void)compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_param_rejects_derived_class_field_slot() {
  // `var` parameters can write back into the caller's slot, so Pascal requires
  // the actual storage to have the formal's exact class type. A derived-class
  // field slot cannot be viewed as a writable base-class slot: the callee could
  // store a different base instance into storage whose declared type is still
  // the derived class.
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      ...
      "end.\n");
  CHECK(error_count() > before);
  CHECK(contains(out.impl, "no matching call to 'visit'"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_storage_ref<t_tnode*>"));
}
...
void test_metaclass_cast_parameterless_constructor_uses_pointer_member() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end;\n"
      "end.\n");
  CHECK(contains(out.impl, "->p_create()"));
  CHECK(!contains(out.impl, ").p_create"));
}
...
void test_method_definition_on_class_alias_uses_canonical_owner() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  CHECK(!contains(out.header, "using t_talias"));
  CHECK(contains(out.impl, "void t_treal::p_ping(t_treal::t_tinner p_x)"));
  CHECK(contains(out.impl, "p_x.p_value = 1;"));
...
void test_metaclass_overloaded_constructors_keep_distinct_slots() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "  b := cls.create(7);\n"
      "end.\n");
  CHECK(contains(out.header, "t_tnode* (*p_create_tp2cc_slot_0)();"));
  CHECK(contains(out.header, "t_tnode* p_create() const"));
  CHECK(contains(out.header,
                 "t_tnode* (*p_create_tp2cc_slot_1)(int32_t);"));
  CHECK(contains(out.header, "t_tnode* p_create(int32_t p_n) const"));
  CHECK(contains(out.impl, "p_a = p_cls->p_create();"));
  CHECK(contains(out.impl, "p_b = p_cls->p_create(7);"));
}
...
void test_inheritsfrom_uses_runtime_tclass_and_method_call() {
  ...
  CHECK(contains(
      out.header,
      "bool p_isbase(t_tbase* p_x, ::rt::tp2cc_metaclass_t_tobject* p_c);"));
...
void test_object_pointer_typecast_deref_method_keeps_receiver_identity() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end.\n");
  CHECK(contains(out.impl, "::rt::tp2cc_deref("));
  CHECK(contains(out.impl, "static_cast<t_pobj>(p_p)"));
  CHECK(contains(out.impl, ").p_reset();"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_load<t_tobj>"));
  CHECK(!contains(out.impl, "tp2cc_storage_ref<t_tobj>"));
}

void test_object_pointer_field_typecast_deref_method_keeps_receiver_identity() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end;\n"
      "end.\n");
  CHECK(contains(out.impl, "static_cast<t_punitsymtable>("));
  CHECK(contains(out.impl, ").p_loadrefs();"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_load<t_tunitsymtable>"));
  CHECK(!contains(out.impl, "tp2cc_storage_ref<t_tunitsymtable>"));
}
...
void test_val_var_arg_typecast_uses_scoped_storage_view() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK(contains(
      out.impl,
      "tp2cc_ScopedStorageView<uint32_t, int32_t>((&p_result), (&p_result))"));
}

void test_val_var_arg_imported_alias_typecast_uses_scoped_storage_view() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_arg_accepts_reference_class_view_over_untyped_pointer_deref() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl,
                 "p_addstatement(::rt::tp2cc_storage_ref<t_tstatementnode*>("));
}

void test_untyped_pointer_record_deref_keeps_managed_field_lvalue() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end.\n");
  CHECK(contains(out.impl,
                 "p_setlength(::rt::tp2cc_deref(static_cast<t_pinfo>(p_arg)).p_values, 1)"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_load<::rt::tp2cc_DynArray"));
  CHECK(!contains(out.impl, "tp2cc_storage_ref<::rt::tp2cc_DynArray"));
}

void test_pointer_slot_cast_from_default_property_keeps_storage_address() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
...
void test_pointer_cast_of_packed_field_address_uses_raw_storage_address() {
  ...
  auto out = compile_snippet_with_registry(
      ...
      "end.\n");
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "offsetof(t_trec, p_value)"));
  CHECK(!contains(out.impl, "static_cast<uint8_t*>(nullptr)"));
}

void test_procedural_actual_accepts_integer_longint_builtin_alias() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
...
void test_var_arg_accepts_explicit_class_cast_slot() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl,
                 "p_take((*::rt::tp2cc_ScopedStorageView<t_tsym*, t_tlabel*>((&p_l), (&p_l))))"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_storage_ref<t_tsym*>"));
}

void test_var_arg_derived_class_slot_is_rejected() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK(error_count() > before);
  CHECK(!contains(out.impl, "tp2cc_reinterpret_storage_ref<t_tbase*>"));
}

void test_nested_proc_var_arg_accepts_explicit_class_cast_slot() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
  ...
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl,
                 "p_touch((*::rt::tp2cc_ScopedStorageView<t_tsym*, t_tlabel*>((&p_l), (&p_l))))"));
  CHECK(!contains(out.impl, "tp2cc_reinterpret_storage_ref<t_tsym*>"));
}
...
void test_shadowed_new_and_dispose_statements_call_user_procs() {
  int before = error_count();
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      ...
      "end.\n");
  CHECK_EQ(error_count(), before);
  CHECK(contains(out.impl, "p_new(1);"));
  CHECK(contains(out.impl, "p_dispose(2);"));
  CHECK(!contains(out.impl, "::rt::p_new("));
  CHECK(!contains(out.impl, "::rt::p_dispose("));
}
...
void test_expression_new_uses_declared_type_named_like_old_backend_stub() {
  auto out = compile_snippet_with_registry(
      "unit u;\n"
      "interface\n"
      "type\n"
      ...
      "end.\n");
  CHECK(contains(out.impl, "::rt::p_new(tp2cc_ptr);"));
  CHECK(contains(out.impl, "return tp2cc_ptr;"));
...
void test_reference_class_alias_cast_uses_payload_carrier() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  CHECK(!contains(out.header, "using t_tinstr"));
...
void test_addr_of_reference_class_typecast_field_uses_object_pointer() {
  auto out = compile_snippet_with_registry(
      ...
      "end;\n"
      "end.\n");
  // `@tchild(p).value` must go through the downcast object pointer, not
  // through `&p_p` (which would be the address of the parameter slot).
  // Any valid emission that starts from `static_cast<t_tchild*>(p_p)` and
  // reaches the `value` field is acceptable; the negative assertion pins
  // down the failure mode we actually care about (accidentally taking
  // `&p_p` as the containing-object address).
  CHECK(contains(out.impl, "static_cast<t_tchild*>(p_p)"));
  CHECK(contains(out.impl, "p_value"));
...
int main() {
  ...
  RUN_TEST(test_program_runs_exitproc_before_returning_exitcode);
  ...
  RUN_TEST(test_unimported_qualified_enum_member_is_not_const_folded);
  ...
  RUN_TEST(test_type_alias_does_not_emit_backend_carrier);
  RUN_TEST(test_build_resolves_alias_descriptor_identity);
  RUN_TEST(test_build_resolves_metaclass_alias_descriptor_identity);
  RUN_TEST(test_runtime_tclass_is_rt_metaclass_alias_not_builtin_atom);
  RUN_TEST(test_runtime_alias_types_come_from_registry_tree);
  RUN_TEST(test_inferred_types_are_registry_owned);
  RUN_TEST(test_descriptor_owned_semantic_indexes);
  ...
  RUN_TEST(test_build_class_forward_completion_can_cross_type_sections);
  RUN_TEST(test_build_class_forward_completion_must_be_class_body);
  RUN_TEST(test_build_class_field_uses_prior_forward_class_decl);
  RUN_TEST(test_build_rejects_implementation_type_redeclaring_interface_type);
  RUN_TEST(test_build_rejects_class_field_before_unseen_later_class);
  RUN_TEST(test_build_var_type_does_not_see_later_type_section);
  RUN_TEST(test_build_proc_local_var_type_does_not_see_later_type_section);
  RUN_TEST(test_build_rejects_proc_local_duplicate_type);
  RUN_TEST(test_build_rejects_proc_local_duplicate_enum_label);
  RUN_TEST(test_build_rejects_proc_local_enum_label_redeclared_by_later_type);
  RUN_TEST(test_build_rejects_proc_local_enum_label_conflicting_with_value_decl);
  RUN_TEST(test_build_rejects_unit_enum_label_conflicting_with_value_decl);
  RUN_TEST(test_build_resolves_set_element_descriptor_identity);
  RUN_TEST(test_build_resolves_builtin_descriptor_identity);
  RUN_TEST(test_builtin_pascal_aliases_share_declared_target_identity);
  ...
  RUN_TEST(test_chr_storage_view_for_byte_assignment_and_inc);
  ...
  RUN_TEST(test_imported_default_argument_does_not_see_caller_result);
  RUN_TEST(test_unit_qualified_variable_assignment_is_storage_designator);
  RUN_TEST(test_unit_qualified_value_type_guides_overload_selection);
  ...
  RUN_TEST(test_build_binds_empty_exception_handler_to_runtime_exception);
  ...
  RUN_TEST(test_shortstring_pointer_cast_reads_only_length_prefixed_storage);
  ...
  RUN_TEST(test_untyped_file_var_method_accepts_file_actual);
  RUN_TEST(test_file_kinds_use_distinct_runtime_carriers);
  ...
  RUN_TEST(test_system_pointer_aliases_are_implicit_source_types);
  ...
  RUN_TEST(test_unit_storage_duplicates_report_error);
  RUN_TEST(test_duplicate_type_members_report_error);
  RUN_TEST(test_sizeof_own_implementation_private_qualified_names);
  RUN_TEST(test_unit_qualified_type_does_not_see_local_type_scope);
  ...
  RUN_TEST(test_absolute_pointer_target_reinterprets_pointer_slot_storage);
  ...
  RUN_TEST(test_shadowed_prefetch_statement_calls_user_proc);
  ...
  RUN_TEST(test_real_division_uses_registered_real_result_type);
  RUN_TEST(test_real_division_preserves_floating_operand_result_type);
  ...
  RUN_TEST(test_untyped_const_pointer_cast_from_integer_passes_pointed_address);
  RUN_TEST(test_untyped_const_pointer_cast_deref_from_integer_passes_pointee_storage);
  RUN_TEST(test_untyped_const_pointer_self_deref_passes_receiver_storage);
  RUN_TEST(test_pointer_cast_deref_from_integer_loads_pointee_storage);
  ...
  RUN_TEST(test_forward_pointer_to_set_emits_set_carrier_first);
  ...
  RUN_TEST(test_interface_parameterless_method_result_autocalls_as_argument);
  ...
  RUN_TEST(test_class_actual_rejects_same_leaf_nested_interface_formal);
  RUN_TEST(test_class_constructor_call_allocates_instance);
  RUN_TEST(test_nested_record_type_emits_inside_owner_scope);
  RUN_TEST(test_record_field_does_not_see_later_nested_type);
  RUN_TEST(test_nested_class_type_emits_qualified_owner_and_method_scope);
  RUN_TEST(test_class_member_does_not_see_later_nested_type);
  ...
  RUN_TEST(test_class_value_rejects_unrelated_class_of_target);
  ...
  RUN_TEST(test_include_mutates_variant_payload_set_storage);
  RUN_TEST(test_include_mutates_typed_pointer_cast_field_set_storage);
  ...
  RUN_TEST(test_var_param_accepts_same_pointee_pointer_alias);
  RUN_TEST(test_var_param_accepts_nested_same_pointee_pointer_alias);
  RUN_TEST(test_var_param_accepts_imported_same_pointee_pointer_alias);
  RUN_TEST(test_var_param_accepts_same_domain_subrange_alias);
  RUN_TEST(test_var_param_accepts_same_shape_fixed_array_alias);
  RUN_TEST(test_var_param_accepts_same_shape_shortstring_array_alias);
  RUN_TEST(test_var_param_rejects_different_shortstring_array_capacity);
  RUN_TEST(test_var_param_accepts_same_shape_set_alias);
  RUN_TEST(test_object_pointer_formal_uses_declaring_unit_context);
  RUN_TEST(test_var_param_rejects_pointer_alias_to_distinct_record);
  ...
  RUN_TEST(test_single_candidate_call_accepts_enum_member_for_subrange);
  ...
  RUN_TEST(test_bare_proc_type_does_not_see_hidden_imported_function);
  ...
  RUN_TEST(test_with_field_type_shadows_current_class_for_overload);
  ...
  RUN_TEST(test_inline_anon_enum_var_name_is_not_type_name);
  ...
  RUN_TEST(test_var_param_rejects_derived_class_field_slot);
  ...
  RUN_TEST(test_metaclass_cast_parameterless_constructor_uses_pointer_member);
  ...
  RUN_TEST(test_metaclass_overloaded_constructors_keep_distinct_slots);
  ...
  RUN_TEST(test_object_pointer_typecast_deref_method_keeps_receiver_identity);
  RUN_TEST(test_object_pointer_field_typecast_deref_method_keeps_receiver_identity);
  RUN_TEST(test_statement_context_member_destroy_autocalls);
  RUN_TEST(test_val_var_arg_typecast_uses_scoped_storage_view);
  RUN_TEST(test_val_var_arg_imported_alias_typecast_uses_scoped_storage_view);
  RUN_TEST(test_var_arg_accepts_reference_class_view_over_untyped_pointer_deref);
  RUN_TEST(test_untyped_pointer_record_deref_keeps_managed_field_lvalue);
  RUN_TEST(test_pointer_slot_cast_from_default_property_keeps_storage_address);
  RUN_TEST(test_pointer_cast_of_packed_field_address_uses_raw_storage_address);
  RUN_TEST(test_procedural_actual_accepts_integer_longint_builtin_alias);
  RUN_TEST(test_var_arg_accepts_explicit_class_cast_slot);
  RUN_TEST(test_var_arg_derived_class_slot_is_rejected);
  RUN_TEST(test_nested_proc_var_arg_accepts_explicit_class_cast_slot);
