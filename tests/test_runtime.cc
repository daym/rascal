...
int exit_proc_order[4] = {};
int exit_proc_count = 0;
int exit_proc_ioresult[4] = {};
ExitProc saved_exit_proc = nullptr;
int exit_proc_child_fd = -1;
int exit_proc_child_code = -1;

void exit_proc_tail() {
  exit_proc_order[exit_proc_count] = 2;
  exit_proc_ioresult[exit_proc_count] = tp2cc_last_ioresult;
  ++exit_proc_count;
}

void exit_proc_head() {
  exit_proc_order[exit_proc_count] = 1;
  exit_proc_ioresult[exit_proc_count] = tp2cc_last_ioresult;
  ++exit_proc_count;
  tp2cc_last_ioresult = 99;
  p_exitproc = saved_exit_proc;
}

void exit_proc_child_handler() {
  const char marker = 'x';
  if (exit_proc_child_fd >= 0) {
    (void)::write(exit_proc_child_fd, &marker, 1);
  }
  if (exit_proc_child_code >= 0) {
    p_exitcode = exit_proc_child_code;
  }
}
...
void test_binary_reset_uses_pascal_filemode() {
  static_assert(std::is_same_v<decltype(p_filemode), uint8_t>);
  CHECK_EQ(p_filemode, static_cast<uint8_t>(2));

  char path[] = "/tmp/tp2cc-filemode-XXXXXX";
  int fd = ::mkstemp(path);
  CHECK(fd >= 0);
  if (fd < 0) return;
  CHECK_EQ(::write(fd, "abc", 3), static_cast<ssize_t>(3));
  ::close(fd);

  tp2cc_UntypedFile untyped{};
  p_assign(untyped, tp2cc_shortstring_of<>(path));
  p_filemode = 1;
  p_reset(untyped, 1);
  CHECK(untyped.f != nullptr);
  if (untyped.f) {
    CHECK_EQ(::fcntl(::fileno(untyped.f), F_GETFL) & O_ACCMODE, O_WRONLY);
    int32_t transferred = 0;
    const char replacement = 'X';
    p_blockwrite(untyped, static_cast<const void*>(&replacement), 1,
                 transferred);
    CHECK_EQ(transferred, 1);
    p_close(untyped);
  }
  ...
  if (check) {
    char contents[4] = {};
    CHECK_EQ(std::fread(contents, 1, 3, check), static_cast<std::size_t>(3));
    CHECK_EQ(std::string(contents, 3), std::string("Xbc"));
    std::fclose(check);
  }

  tp2cc_TypedFile<uint8_t> typed{};
  p_assign(typed, tp2cc_shortstring_of<>(path));
  p_filemode = 2;
  p_reset(typed);
  CHECK(typed.f != nullptr);
  if (typed.f) {
    CHECK_EQ(::fcntl(::fileno(typed.f), F_GETFL) & O_ACCMODE, O_RDWR);
    p_close(typed);
  }
  ...
  CHECK(untyped.f != nullptr);
  if (untyped.f) {
    CHECK_EQ(::fcntl(::fileno(untyped.f), F_GETFL) & O_ACCMODE, O_RDONLY);
    p_close(untyped);
  }

  tp2cc_TextFile text{};
  p_assign(text, tp2cc_shortstring_of<>(path));
  p_filemode = 1;
  p_reset(text);
  CHECK(text.f != nullptr);
  if (text.f) {
    CHECK_EQ(::fcntl(::fileno(text.f), F_GETFL) & O_ACCMODE, O_RDONLY);
    p_close(text);
  }
...
void test_predefined_text_files_use_process_standard_streams() {
  CHECK(p_input.f == stdin);
  CHECK(p_output.f == stdout);
  CHECK(p_stderr.f == stderr);
}

void test_exitproc_drains_restored_handlers_in_order() {
  exit_proc_count = 0;
  exit_proc_order[0] = exit_proc_order[1] = 0;
  exit_proc_ioresult[0] = exit_proc_ioresult[1] = -1;
  saved_exit_proc = &exit_proc_tail;
  p_exitproc = &exit_proc_head;
  tp2cc_last_ioresult = 47;

  tp2cc_run_exit_procs();

  CHECK_EQ(exit_proc_count, 2);
  CHECK_EQ(exit_proc_order[0], 1);
  CHECK_EQ(exit_proc_order[1], 2);
  CHECK_EQ(exit_proc_ioresult[0], 0);
  CHECK_EQ(exit_proc_ioresult[1], 0);
  CHECK(p_exitproc == nullptr);
  saved_exit_proc = nullptr;
}

void test_halt_and_runerror_invoke_exitproc() {
  auto run_child = [](bool runerror, int requested_code,
                      int handler_code) {
    int pipe_fds[2] = {-1, -1};
    ...
    if (pid == 0) {
      ::close(pipe_fds[0]);
      exit_proc_child_fd = pipe_fds[1];
      exit_proc_child_code = handler_code;
      p_exitproc = &exit_proc_child_handler;
      if (runerror) {
        p_runerror(requested_code);
      } else {
        p_halt(requested_code);
      }
    }

    ::close(pipe_fds[1]);
    char marker = '\0';
    CHECK_EQ(::read(pipe_fds[0], &marker, 1), static_cast<ssize_t>(1));
    ::close(pipe_fds[0]);
    int status = 0;
    CHECK_EQ(::waitpid(pid, &status, 0), pid);
    CHECK(WIFEXITED(status));
    if (WIFEXITED(status)) {
      const int expected = handler_code >= 0 ? handler_code : requested_code;
...
int main() {
  ...
  RUN_TEST(test_binary_reset_uses_pascal_filemode);
  RUN_TEST(test_predefined_text_files_use_process_standard_streams);
  RUN_TEST(test_exitproc_drains_restored_handlers_in_order);
  RUN_TEST(test_halt_and_runerror_invoke_exitproc);
