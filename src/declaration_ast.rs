...
pub enum TypeSyntaxKind {
    ...
    /// An external selector carried by the RTL declaration. Semantic binding
    /// decides the Pascal type; the frontend must not interpret this string as
    /// a Pascal identifier.
    External {
        backend_name: String,
    },
...
pub struct ValueDeclarationSyntax {
    ...
    /// Raw selector from `external name '...'`.
...
pub struct RoutineDeclarationSyntax {
    ...
    /// Raw selector from `external name '...'`. It may select a hardcoded
    /// compiler builtin or a backend external, but never participates in
    /// Pascal name lookup.
    pub external_name: Option<String>,
    pub overload: bool,
